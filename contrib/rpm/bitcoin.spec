...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5518
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5519
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15518
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15519
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5518
	%{_sbindir}/semanage port -d -p tcp 5519
	%{_sbindir}/semanage port -d -p tcp 15518
	%{_sbindir}/semanage port -d -p tcp 15519
	for selinuxvariant in %{selinux_variants}; do
