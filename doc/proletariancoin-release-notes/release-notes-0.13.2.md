Proletariancoin Core version 0.13.2 is now available from:

  <https://download.proletariancoin.cf/proletariancoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a proletariancoin
...
ECDSA signatures inside Proletariancoin transactions now use validation using
...
Previous versions of Proletariancoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Proletariancoin Core was to raise the default minimum
relay fee.

Proletariancoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Proletariancoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running proletariancoind without having to do any manual configuration.
...
Proletariancoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Proletariancoin Core will relay transactions with
...
entirely in Proletariancoin Core 0.13, and it is at this time undecided whether the
...
Proletariancoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Proletariancoin Core automatically creates a hidden service to listen on, without
manual configuration. Proletariancoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Proletariancoin Core is listening, and
...
Proletariancoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Proletariancoin
Core to use floating fees. By default, Proletariancoin Core will use floating
fees.
...
At all times, Proletariancoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PRC.
Furthermore, Proletariancoin Core will never create transactions paying less than
...
`proletariancoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 25518 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 25518 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /proletariancoinrpc>
        ProxyPass http://127.0.0.1:5518/
        ProxyPassReverse http://127.0.0.1:5518/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass proletariancoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< proletariancoinrpc:<password>
    </Location>
...
Restarting proletariancoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in proletariancoin.conf
...
proletariancoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/proletariancoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Proletariancoin Core code base has
...
- `proletariancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `proletariancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Proletariancoin scripting system that in combination with
...
Proletariancoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Proletariancoin Core.
...
The two phases are distinct in the Proletariancoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Proletariancoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Proletariancoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Proletariancoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of proletariancoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of proletariancoins being spent is exactly
...
- **Increased security for multisig:** Proletariancoin addresses (both P2PKH addresses
  ...
  believed to be provided by Proletariancoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Proletariancoin's current ledger (called the UTXO set),
  ...
  note that Proletariancoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Proletariancoin users to individually opt-in to almost any change in the Proletariancoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Proletariancoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Proletariancoin
client to Proletariancoin Core 0.13.2 to signal for segwit support. When a
...
Since Proletariancoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `proletariancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `proletariancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
