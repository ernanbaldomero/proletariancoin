Proletariancoin Core version *0.15.1* is now available from:

  <https://download.proletariancoin.cf/proletariancoin-0.15.1/>
...
A number of changes to the way Proletariancoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 capital per kB (0.00001 PRC). This is to prepare for dropping the minimum transaction fee to 0.1 capital per kB (0.0001 PRC) in the next release.
